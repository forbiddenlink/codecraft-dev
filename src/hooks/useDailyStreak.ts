...
const STREAK_MILESTONES: Omit<StreakMilestone, 'claimed'>[] = [
  ...
  {
    day: 30,
    reward: { type: 'cosmetic', value: 'golden_building', label: 'Golden Building Skin' },
  },
  { day: 60, reward: { type: 'badge', value: 'master', label: 'Code Master Badge' } },
  {
    day: 100,
    reward: { type: 'cosmetic', value: 'legendary_aura', label: 'Legendary Aura Effect' },
  },
]
...
export function useDailyStreak() {
  ...
  const claimMilestone = useCallback((day: number) => {
    setMilestones((prev) => prev.map((m) => (m.day === day ? { ...m, claimed: true } : m)))
  }, [])
  ...
  const daysUntilNextMilestone = nextMilestone ? nextMilestone.day - streakData.currentStreak : null
