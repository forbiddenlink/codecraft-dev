...
let wcInstance: Awaited<ReturnType<typeof import('@webcontainer/api').WebContainer.boot>> | null =
  null
let bootPromise: Promise<
  Awaited<ReturnType<typeof import('@webcontainer/api').WebContainer.boot>>
> | null = null
...
export function useWebContainerSandbox(): UseWebContainerSandboxReturn {
  ...
  const run = useCallback(async (code: string, expected?: string): Promise<ExecutionResult> => {
    setIsRunning(true)
    setError(null)
    ...
    try {
      ...
      const exitCode = await Promise.race<number>([
        ...
        new Promise<number>((_, reject) =>
          setTimeout(() => reject(new Error('Execution timed out after 8s')), TIMEOUT_MS)
        ),
      ])

      const durationMs = Date.now() - start
      const didPass = expected !== undefined ? stdout.trim() === expected.trim() : exitCode === 0
