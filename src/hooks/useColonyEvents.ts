...
import { forceColonyEvent, triggerColonyEvent } from '@/store/slices/eventSlice'
...
export function useColonyEvents() {
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      const coreLoopReady = completedChallenges.length > 0 && placedBuildings.length > 0
      if (!coreLoopReady) {
  ...
  }, [activeEventId, tutorialActive, playerLevel, placedBuildings, completedChallenges, dispatch])
}
