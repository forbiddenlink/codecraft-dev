...
function MissionConsole({ onSuccess }: { onSuccess?: () => void }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-b-md bg-zinc-950 px-4 py-2">
      <span className="text-xs text-zinc-400 font-mono">COLONY TERMINAL v2.1</span>
    </div>
  )
}
...
export function ChallengeEditor({ challenge, onComplete }: ChallengeEditorProps) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-zinc-800 bg-zinc-950">
      ...
      <div className="border-b border-zinc-800 bg-gradient-to-r from-cyan-950/50 to-zinc-900 p-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <h3 className="mt-0.5 text-lg font-bold text-white">{challenge.missionTitle}</h3>
          </div>
          <div className="rounded-full bg-yellow-500/10 px-3 py-1 text-sm font-medium text-yellow-400">
            +{challenge.xpReward} XP
          </div>
        </div>
        <p className="mt-2 text-sm text-zinc-400 italic">&ldquo;{challenge.briefing}&rdquo;</p>
      </div>
      ...
      >
        <SandpackLayout>
          <SandpackCodeEditor style={{ height: 380 }} showLineNumbers showInlineErrors />
