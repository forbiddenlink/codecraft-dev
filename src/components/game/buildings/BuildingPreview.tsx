...
import { cancelPlacement, rotatePreview, updatePreviewPosition } from '@/store/slices/buildingSlice'
...
export default function BuildingPreview({ gridSnap = true }: BuildingPreviewProps) {
  ...
  const resolvedTemplate = selectedTemplateId ? buildingTemplates[selectedTemplateId] : null
  ...
  useFrame((state) => {
    ...
    const groundIntersect = intersects.find((intersect) => intersect.object.name === 'ground')
