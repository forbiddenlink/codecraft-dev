...
export default function PixelEmissiveParticles({
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      trail[i3 + 1] =
        trail[i3 + 1] * behavior.trail + positions.array[i3 + 1] * (1 - behavior.trail)
      trail[i3 + 2] =
        trail[i3 + 2] * behavior.trail + positions.array[i3 + 2] * (1 - behavior.trail)
  ...
  return (
    <group>
      ...
      <points>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[initialTrail, 3]} />
        </bufferGeometry>
        <pointsMaterial
