  ...
  reducers: {
    ...
    removeGenerator: (state, action: PayloadAction<string>) => {
      ...
      if (generator) {
        ...
        state.productionRates[generator.resourceType] -= generator.outputRate * generator.efficiency
      }
    ...
    addResources: (
      ...
      state.storage[type] = Math.min(state.storage[type] + amount, state.capacity[type])
    },
    ...
    consumeResources: (
      ...
      const { type, amount } = action.payload
      state.storage[type] = Math.max(state.storage[type] - amount, 0)
    },
    ...
    processTick: (state, action: PayloadAction<number>) => {
      ...
      Object.entries(state.consumptionRates).forEach(([resource, rate]) => {
        ...
        state.storage[resourceType] = Math.max(state.storage[resourceType] - consumption, 0)
      })
    },
  },
})
