...
import type { BuildingEffect } from '@/data/buildingTemplates'
...
import { validateCss, validateHtml, validateJs } from '@/utils/codeValidation'
import { applyStyles, parseCSSRule } from '@/utils/cssParser'
import { type GameStructureNode, parseHtmlToStructure } from '@/utils/htmlParser'
...
export class CodeToGamePipeline {
  ...
  processCode(html: string, css: string, js?: string) {
    ...
    if (!htmlValidation.isValid || !cssValidation.isValid || !jsValidation.isValid) {
      return {
        ...
        buildings: [],
        behaviors: {},
        errors: [...htmlValidation.errors, ...cssValidation.errors, ...(jsValidation.errors || [])],
        warnings: [
    ...
    if (node.styles?.transform?.includes('scale')) {
      ...
      if (scaleMatch && scaleMatch[1]) {
        ...
        if (scaleValues.length === 1) {
          ...
          // Non-uniform scale
          return [scaleValues[0], scaleValues[1], scaleValues.length > 2 ? scaleValues[2] : 1]
        }
      }
    }
  ...
  private updateGameState(
    styledStructure: ExtendedGameStructureNode[],
