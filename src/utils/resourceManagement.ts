...
class ResourceManagementSystem {
  ...
  getChainEfficiency(chainId: string): number {
    ...
    const totalEfficiency = buildings.reduce((sum, b) => sum + (b.isActive ? b.efficiency : 0), 0)
  ...
  getOptimizationSuggestions(): string[] {
    ...
    this.resources.forEach((resource) => {
      ...
      if (Math.abs(netRate) < 1) {
        suggestions.push(`⚖️ ${resource.type} production is balanced. Good job!`)
      }
    })
...
export const getResourceForecast = (type: ResourceType) => getResourceManager().getForecast(type)
export const getAllResourceForecasts = () => getResourceManager().getAllForecasts()
export const getOptimizationSuggestions = () => getResourceManager().getOptimizationSuggestions()
