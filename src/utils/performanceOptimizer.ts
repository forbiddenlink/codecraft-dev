...
export class PerformanceOptimizer {
  ...
  optimizeRenderer(renderer: THREE.WebGLRenderer): void {
    ...
    renderer.shadowMap.type = this.fps >= 55 ? THREE.PCFSoftShadowMap : THREE.BasicShadowMap
   ...
   */
  createObjectPool<T>(factory: () => T, reset: (obj: T) => void, initialSize: number = 10) {
    const pool: T[] = []
   ...
   */
  createAnimationLoop(callback: (deltaTime: number) => void, targetFPS: number = 60): () => void {
