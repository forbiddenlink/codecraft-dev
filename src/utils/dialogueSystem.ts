...
class DialogueSystem {
   ...
   */
  advanceDialogue(treeId: string, currentNodeId: string, choiceId?: string): DialogueNode | null {
    ...
    effects.forEach((effect) => {
      switch (effect.type) {
        ...
        case 'giveItem': {
          ...
          this.playerState.inventory.set(effect.param as string, currentCount + (effect.value || 1))
          break
        }
   ...
   */
  private recordHistory(npcId: string, treeId: string, nodeId: string, choiceId?: string): void {
...
export const startConversation = (npcId: string) => getDialogueSystem().startConversation(npcId)
...
export const getNPCRelationship = (npcId: string) => getDialogueSystem().getRelationship(npcId)
