...
class ProgressiveHintSystem {
  ...
  shouldShowAutoHint(challengeId: string): boolean {
    ...
    return strategy.timeStuck >= this.config.timeForAutoHint && strategy.currentLevel < 3
  }
...
class VisualGoalSystem {
   ...
   */
  compareTocTarget(userHTML: string, userCSS: string, target: VisualGoal): DiffResult {
    ...
    const totalChecks =
      ...
      Object.values(targetStyles).reduce((sum, props) => sum + Object.keys(props).length, 0)
  ...
  private elementsMatch(el1: HTMLElement, el2: HTMLElement): boolean {
    return el1.tagName === el2.tagName && el1.className === el2.className && el1.id === el2.id
  }
...
export const getNextHint = (challengeId: string) => getHintSystem().getNextHint(challengeId)
export const recordAttempt = (challengeId: string) => getHintSystem().recordAttempt(challengeId)
...
export const generateVisualHints = (diff: DiffResult) => getVisualGoalSystem().generateHints(diff)
