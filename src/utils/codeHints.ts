...
export class CodeHintsSystem {
  private htmlTags: CodeHint[] = [
    {
      ...
      insertText: '<header>\n  \n</header>',
      documentation:
        'The <header> element represents introductory content, typically containing heading elements, logos, or navigation.',
      example: '<header>\n  <h1>My Website</h1>\n  <nav>...</nav>\n</header>',
      difficulty: 1,
    },
    {
  ...
  private cssProperties: CodeHint[] = [
    ...
    },
    {
      ...
      documentation:
        "The padding property creates space around an element's content, inside any defined borders.",
    ...
    },
    {
      ...
      documentation:
        'The margin property creates space around an element, outside any defined borders.',
  ...
  getHints(context: ContextAnalysis, userLevel: number = 1): CodeHint[] {
    ...
    if (context.language === 'html') {
      if (context.inAttribute) {
        // Suggest attributes
        hints = this.htmlAttributes.filter((h) => h.difficulty! <= userLevel + 1)
      } else if (context.inTag) {
        // Already in a tag, suggest attributes
        hints = this.htmlAttributes
      } else {
        // Suggest HTML tags
        hints = this.htmlTags.filter((h) => h.difficulty! <= userLevel + 1)
      }
    } else if (context.language === 'css') {
      // Suggest CSS properties
      hints = this.cssProperties.filter((h) => h.difficulty! <= userLevel + 1)
    } else if (context.language === 'javascript') {
      // Suggest JavaScript functions and keywords
      hints = this.jsFunctions.filter((h) => h.difficulty! <= userLevel + 1)
    }

    // Filter by current input
    const currentWord = this.getCurrentWord(context.currentLine, context.cursorPosition)
    if (currentWord) {
      hints = hints.filter((h) => h.text.toLowerCase().startsWith(currentWord.toLowerCase()))
    }
...
export function useCodeHints(code: string, cursorPosition: number, userLevel: number = 1) {
