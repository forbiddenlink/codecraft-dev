...
import { validateCss, validateHtml, validateJs } from './codeValidation'
import { parseCSSRule } from './cssParser'
import { getAllNodes, parseHtmlToStructure } from './htmlParser'
...
function convertChallengeToGradingCriteria(
  criteria: ChallengeGradingCriteria[]
): GradingCriteria[] {
 ...
 */
export function gradeJavaScript(
  challenge: ChallengeWithGrading,
  code: string
): SimplifiedGradingResult {
  const criteria = convertChallengeToGradingCriteria(challenge.gradingCriteria)
...
export const createCriteria = {
  ...
  hasElementWithAttribute: (
    ...
    const criteria = CriteriaBuilders.hasElementWithAttribute(
      tagName,
      attrName,
      undefined,
      description
