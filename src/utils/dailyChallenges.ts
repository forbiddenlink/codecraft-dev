...
class DailyChallengeSystem {
  ...
  private generateChallenge(difficulty: DailyChallenge['difficulty']): Challenge {
    ...
    return {
      ...
      category: 'structure',
      difficulty:
        difficulty === 'expert' ? 3 : difficulty === 'hard' ? 3 : difficulty === 'medium' ? 2 : 1,
      requiredChallenges: [],
      htmlTemplate: '',
      cssTemplate: '',
...
class WeeklyMissionSystem {
  ...
  updateProgress(objectiveType: WeeklyMission['objectives'][0]['type'], amount: number): void {
    ...
    if (objective) {
      ...
      const allCompleted = this.currentMission.objectives.every((obj) => obj.current >= obj.target)
      if (allCompleted) {
...
class StreakSystem {
  ...
  getAvailableMilestones(): Streak['milestones'] {
    return this.streak.milestones.filter((m) => !m.claimed && this.streak.current >= m.days)
  }
}
